...
## Gemma 4 26B (MoE) on 4× RTX 4090 with TensorRT-LLM (Ubuntu or CentOS 9)

End-to-end guide for deploying [google/gemma-4-26b-it](https://huggingface.co/google/gemma-4-26b-it) using
...
**Architecture note:** Gemma 4 26B is a Mixture-of-Experts (MoE) model with 26B total parameters and
4B active parameters per token. All 26B parameters must be loaded into VRAM at runtime.
With TP=4 across four RTX 4090s, each GPU holds ~12 GB of model weights, leaving ~12 GB per GPU
...
| System RAM | 64 GB |
| Disk | 80 GB free (model weights ≈ 48 GB in BF16) |
...
All scripts live in `scripts/gemma4-26b/`. Run them in the order shown below.
...
|----------|---------|-------------|
| `PIP_INDEX_URL` | `https://pypi.tuna.tsinghua.edu.cn/simple` | Mirror used for PyPI packages |
...
Gemma 4 is a **gated model**. Before running this step you must:
...
HF_ENDPOINT=https://hf-mirror.com bash scripts/gemma4-26b/download_model.sh
```

To use the official endpoint instead of the China mirror:

```bash
HF_ENDPOINT=https://huggingface.co bash scripts/gemma4-26b/download_model.sh
```

| Variable | Default | Description |
|----------|---------|-------------|
| `MODEL_DIR` | `./models/gemma-4-26b-it` | Local path for downloaded weights |
| `HF_MODEL_ID` | `google/gemma-4-26b-it` | Hugging Face model repository |
| `HF_TOKEN` | *(required)* | HF access token — mandatory for Gemma 4 |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `MODEL_DIR` | `./models/gemma-4-26b-it` | Source HF model directory |
| `CKPT_DIR` | `./checkpoints/gemma-4-26b-tp4` | Output directory |
| `TRTLLM_REPO` | `./TensorRT-LLM` | TensorRT-LLM repository (auto-cloned if absent) |
| `TP_SIZE` | `4` | Tensor parallel degree |
| `EP_SIZE` | `1` | Expert parallel degree (set to `4` to shard MoE experts across GPUs) |
...
The `--moe_plugin` flag is enabled automatically to handle the sparse expert-routing layers.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CKPT_DIR` | `./checkpoints/gemma-4-26b-tp4` | Converted checkpoint |
| `ENGINE_DIR` | `./engines/gemma-4-26b-tp4` | Output engine directory |
| `TP_SIZE` | `4` | Tensor parallel degree |
| `MAX_BATCH_SIZE` | `4` | Maximum concurrent sequences |
| `MAX_INPUT_LEN` | `8192` | Maximum input tokens |
| `MAX_OUTPUT_LEN` | `2048` | Maximum generated tokens |
| `MAX_NUM_TOKENS` | `16384` | Maximum tokens in flight |

> **Note:** TensorRT engines are GPU-specific. Rebuild if you change hardware.
> To increase throughput, try `MAX_BATCH_SIZE=8` and `MAX_NUM_TOKENS=32768` after
> verifying VRAM headroom with `nvidia-smi`.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `ENGINE_DIR` | `./engines/gemma-4-26b-tp4` | Pre-built engine (used if present) |
| `MODEL_DIR` | `./models/gemma-4-26b-it` | HF model path (tokenizer + fallback) |
...
# Custom prompt
PROMPT="Describe the Gemma 4 MoE architecture." bash scripts/gemma4-26b/run_inference.sh
```

The script calls the `/v1/chat/completions` endpoint with a system prompt (supported natively
in Gemma 4) and prints the model response.
...
|---------|-------------|-----|
| OOM during engine build | `MAX_NUM_TOKENS` too high for MoE weight footprint | Reduce `MAX_NUM_TOKENS` or `MAX_BATCH_SIZE` |
...
| HF download fails or 401 error | Gated model / missing token | Accept licence at `huggingface.co/google/gemma-4-26b-it` and set `HF_TOKEN` |
| Conversion script not found | Outdated TRT-LLM clone | Run `cd TensorRT-LLM && git pull` (requires TRT-LLM 0.20.0+) |
...
└── gemma4-26b/
    ...
    ├── download_model.sh       # Download google/gemma-4-26b-it from Hugging Face
    ├── convert_checkpoint.sh   # Convert HF checkpoint → TRT-LLM format (TP=4, MoE)
    ├── build_engine.sh         # Compile optimised TensorRT engine (moe_plugin enabled)
